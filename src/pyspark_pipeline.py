...
def run_parkinsons_pipeline():
    ...
    # 4. Train/Test Split
    train_spark, test_spark = df_scaled.randomSplit(
        [split_ratio, 1.0 - split_ratio], seed=42
    )
    logger.info(
        f"Data split: Train count={train_spark.count()}, Test count={test_spark.count()}"
    )
    ...
    for model_key in config["pipeline"]["models_to_run"]:
        ...
        if model_key == "lr":
            ...
            model = RandomForestClassifier(
                labelCol="label", featuresCol="features", **params
            )
            model_trained = model.fit(train_spark)
